...
(function () {
      ...
      for (var i = 0; i < settings.length; i++) {
        ...
        if (setting == "vertical") {
          ...
          if (positionAlign !== undefined) {
            if (
              !["line-left", "center", "line-right"].includes(positionAlign)
            ) {
              err(
                "Position alignment needs to be one of line-left, center or line-right",
              );
              continue;
            }
        ...
        if (state == "data") {
          ...
          if (c == "<" || c == undefined) {
            ...
            if ((m = buffer.match(/^&#(x?[0-9]+)$/))) {
              ...
              (m = Object.keys(entities).find((n) => buffer.startsWith(n)))
            ) {
              // partial match
  ...
  var WebVTTSerializer = function () {
    function serializeTimestamp(seconds) {
      const ms = (
        "00" +
        (seconds - Math.floor(seconds)).toFixed(3) * 1000
      ).slice(-3);
      let h = 0,
        m = 0,
        s = 0;
    ...
    function serializeCueSettings(cue) {
      var result = "";
      const nonDefaultSettings = Object.keys(defaultCueSettings).filter(
        (s) => cue[s] !== defaultCueSettings[s],
      );
      ...
      if (
        nonDefaultSettings.includes("lineAlign") ||
        nonDefaultSettings.includes("linePosition")
      ) {
        result +=
          " line:" +
          cue.linePosition +
          (cue.snapToLines ? "" : "%") +
          (cue.lineAlign && cue.lineAlign != defaultCueSettings.lineAlign
            ? "," + cue.lineAlign
            : "");
      }
      if (
        nonDefaultSettings.includes("textPosition") ||
        nonDefaultSettings.includes("positionAlign")
      ) {
        result +=
          " position:" +
          cue.textPosition +
          "%" +
          (cue.positionAlign &&
          cue.positionAlign !== defaultCueSettings.positionAlign
            ? "," + cue.positionAlign
            : "");
      }
      return result;
  ...
  function exportify(object) {
    ...
    object.WebVTTCueTimingsAndSettingsParser =
