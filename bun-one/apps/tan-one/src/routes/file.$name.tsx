import { createFileRoute, Link } from "@tanstack/react-router";
import { formatTimestamp } from "@bun-one/vtt";
import { getVttFile } from "../lib/vtt-server";
import { VttViewer } from "../components/VttViewer";

export const Route = createFileRoute("/file/$name")({
  loader: async ({ params }) => {
    // Feature C: Load VTT file by name
    return await getVttFile({ data: params.name });
  },
  component: DetailPage,
});

function DetailPage() {
  const { filename, cues, summary, error } = Route.useLoaderData();

  if (error) {
    return (
      <div className="min-h-screen bg-slate-900 p-8">
        <div className="max-w-4xl mx-auto">
          <Link
            to="/"
            className="text-cyan-400 hover:text-cyan-300 mb-4 inline-block"
          >
            ← Back
          </Link>
          <div className="bg-red-900/50 border border-red-500 rounded-lg p-4 text-red-200">
            Error loading {filename}: {error}
          </div>
        </div>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-slate-900 p-8">
      <div className="max-w-4xl mx-auto space-y-6">
        {/* Header */}
        <div className="flex items-center justify-between">
          <Link
            to="/"
            className="text-cyan-400 hover:text-cyan-300 flex items-center gap-1"
          >
            ← Back to Dashboard
          </Link>
        </div>

        {/* File Info Card */}
        <div className="bg-slate-800 rounded-xl border border-slate-700 overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-700 flex items-center justify-between">
            <h1 className="text-xl font-bold text-white font-mono">
              {filename}
            </h1>
            {summary && (
              <span
                className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${
                  summary.monotonicityViolations === 0
                    ? "bg-green-900 text-green-300"
                    : "bg-red-900 text-red-300"
                }`}
              >
                {summary.monotonicityViolations === 0
                  ? "Valid"
                  : `${summary.monotonicityViolations} Violations`}
              </span>
            )}
          </div>

          {/* Stats Grid */}
          {summary && (
            <div className="p-6 grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-slate-700/50 rounded-lg p-4">
                <div className="text-sm text-gray-400 mb-1">Duration</div>
                <div className="text-lg font-mono font-semibold text-white">
                  {formatTimestamp(summary.durationSec)}
                </div>
              </div>
              <div className="bg-slate-700/50 rounded-lg p-4">
                <div className="text-sm text-gray-400 mb-1">Cue Count</div>
                <div className="text-lg font-mono font-semibold text-white">
                  {summary.cueCount}
                </div>
              </div>
              <div className="bg-slate-700/50 rounded-lg p-4">
                <div className="text-sm text-gray-400 mb-1">Start</div>
                <div className="text-lg font-mono font-semibold text-white">
                  {summary.firstCueStart}
                </div>
              </div>
              <div className="bg-slate-700/50 rounded-lg p-4">
                <div className="text-sm text-gray-400 mb-1">End</div>
                <div className="text-lg font-mono font-semibold text-white">
                  {summary.lastCueEnd}
                </div>
              </div>
            </div>
          )}
        </div>

        {/* Cues List with Violation Filtering */}
        <VttViewer cues={cues} />
      </div>
    </div>
  );
}
