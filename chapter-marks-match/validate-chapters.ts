...
const WHISPER_EXEC = "whisper-cli"; // Assumed to be in PATH or alias, but we might need absolute path if not
const WHISPER_MODELS = join(WHISPER_HOME, "models");
const WHISPER_MODEL_PATH = join(WHISPER_MODELS, "ggml-base.en.bin");

const AUDIOBOOKS_ROOT = process.env.AUDIOBOOKS_ROOT || "/Volumes/Space/Reading/audiobooks";
const BOOK_DIR = process.env.BOOK_DIR || join(AUDIOBOOKS_ROOT, "Emad Mostaque - The Last Economy");
const BOOKMARK_DURATION_DEFAULT = process.env.BOOKMARK_DURATION_DEFAULT || "4";

const RUN_FFPLAY = process.env.RUN_FFPLAY === "true";
const RUN_WHISPER = process.env.RUN_WHISPER === "true";
...
async function findAudioFile(dir: string): Promise<string | null> {
  const proc = spawn(["find", dir, "-name", "*.m4b", "-type", "f"], { stdout: "pipe" });
...
async function playSegment(start: number, duration: string, label: string, audioFile: string) {
  const ffplayArgs = [
    "ffplay",
    "-ss", start.toString(),
    "-t", duration,
...
async function runWhisper(start: number, duration: string, audioFile: string): Promise<string> {
  const ffmpegArgs = [
    "ffmpeg",
    "-v", "error",
    "-ss", start.toString(),
  ...
  const whisperCmdStr = `${ffmpegArgs.join(" ")} | ${whisperArgs.join(" ")}`;
  // Only log command if we are not in a "quiet" mode, or just log it for debugging
  // console.log(`- whisper command: \`${whisperCmdStr}\``); 
...
function findTitleEndTimestamp(title: string, words: Word[]): number | null {
  const titleWords = title.split(/\s+/).map(normalize).filter(w => w.length > 0);
...
for (const chapter of metadata.chapters) {
  ...
  await playSegment(start, BOOKMARK_DURATION_DEFAULT, `Chapter ${id} Start`, audioFile);
